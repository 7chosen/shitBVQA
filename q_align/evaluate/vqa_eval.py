...
def load_video(video_file):
    ...
    fps = vr.get_avg_fps()
    sample_len = int(len(vr) / fps)
    for start_idx in range(0, len(vr), sample_len):
        end_idx = min(start_idx + sample_len, len(vr))
        # Fetch frames in the range [start_idx:end_idx]
        frames_batch = vr.get_batch(range(start_idx, end_idx)).asnumpy()
        # Append to the list
        tmp = [Image.fromarray(x) for x in frames_batch]
        frames_2d_list.append(tmp)
    return frames_2d_list
    ...
    for image_path, json_ in zip(image_paths, jsons):
        with open(json_) as f:
            ...
            for i, llddata in enumerate(tqdm(iqadata, desc="Evaluating [{}]".format(json_.split("/")[-1]))):
                ...
                image_list = load_video(image_path + filename)
                for image in image_list:
                    ...
                    inp = llddata["conversations"][0]["value"].split("\n")[0]
                    ...
                    with torch.inference_mode():
                        ...
                        tmp_prs.append(llddata["score"])
                        tmp_gts.append(llddata["gt_score"])
                        json_ = json_.replace("combined/", "combined-")
                        with open(f"results/{args.model_path}/{json_.split('/')[-1]}", "a") as wf:
                            json.dump(llddata, wf)

                prs.append(np.mean(tmp_prs))
                gts.append(np.mean(tmp_gts))
                if i>0 and i%200==0:
